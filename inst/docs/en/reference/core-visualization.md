...
| `unit` | Time unit of input tree edge lengths. `"Ga"` (giga-annum) or `"Ma"` (mega-annum). If `"Ga"`, edge lengths are automatically converted to Ma (multiplied by 1000). If `NULL` (default), the tree's native units are left untouched; when `add_timescale = TRUE` the pipeline assumes Ma and issues a unit-sanity warning if the median branch length looks inconsistent (there is no automatic unit detection — specify the unit explicitly for a correct time axis). Default: `NULL`. |
