...
> **Note:** GitHub repository at <https://github.com/zengzichao/Rclade>.
...
When reporting a bug, please include: - A minimal reproducible example -
Your R session info
...
- Run `devtools::document()` after modifying roxygen comments to keep
  `man/` in sync
...
- Update both `README.EN.md` and `README.CN.md` if user-facing behavior
  changes.
- Update `NEWS.EN.md` and `NEWS.CN.md` for notable changes.
- Regenerate `man/` pages with `devtools::document()` when roxygen
  comments change.
